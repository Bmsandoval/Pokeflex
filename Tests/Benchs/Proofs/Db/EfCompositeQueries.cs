...
namespace Tests.Benchs.Proofs.Db
{
    ...
    [BenchmarkCategory("Proofs", "Db", "EfCompositeQueries")]
    public class EfCompositeQueries : EfCompositeQueriesBase
    {
        ...
        // {
        //     var pokeflexContext=DbContext.PokeflexContext;
        //     return pokeflexContext.Pokemons.Where(ExtensionLinqHelpers.PredicateExpression(Group, Number)).FirstOrDefault();
        // }
        //
...
WHERE ([p].[GroupId] = @p0) AND ([p].[Number] = @p1)";
    ...
    public static class CompositeExtensionLinqHelpers
    {
        ...
        public static Expression<Func<Pokemon, bool>> PredicateExpressionCombined(int? group, int number)=>
            PredicateBuilder.True<Pokemon>().And(p => p.GroupId == group && p.Number == number);
        public static Expression<Func<Pokemon, bool>> PredicateExpressionGroup(int? group, int number)=>
            PredicateBuilder.True<Pokemon>().And(p => p.GroupId == group && p.Number == number);
        public static Expression<Func<Pokemon, bool>> PredicateExpressionNumber(int number)=>
            PredicateBuilder.True<Pokemon>().And(p=>p.Number == number);
    ...
    // public static class PokemonQueryableHelper
    // {
    //     public class PokemonQueryable
    //     {
    //         public IQueryable<Pokemon> Queryable;
    //         
    //         public PokemonQueryable 
    //     }
    //     
    //     public static PokemonQueryable WhereP(this IQueryable<Pokemon> q)
    //     {
    //         return new(){Queryable = q};
    //     }
    // }
}
